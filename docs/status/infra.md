...
- [2] `.github/workflows/release.yml`: tag `v*` → APK (assinado via `apksigner` com secrets `ANDROID_*`; sem eles, debug), `.dmg` (create-dmg), `.zip` Windows, SHA256SUMS, `ghcr.io/felipemaion/piriquito-relay:<tag>`+`:latest` multi-arch, `gh release create`.
...
- [4] `docs/ops/RUNBOOK.md` (checklist §9 preenchido: `piriquito01`, `/home/<DOMINIO>/`, bloco Caddy com WS/limites, secrets, `admin bootstrap`), `FIREBASE.md`, `BACKUP.md`.
